    ...
    , thread_pool_(conf.threadpool_size)
{
    ...
    if (conf.ssl.server_cert_path && conf.ssl.server_private_key_path) {
        if (!std::filesystem::exists(*conf.ssl.server_cert_path)) {
            SPDLOG_ERROR("SSL certificate file '", *conf.ssl.server_cert_path, "' not found");
        }

        if (!std::filesystem::exists(*conf.ssl.server_private_key_path)) {
            SPDLOG_ERROR("SSL key file '", *conf.ssl.server_private_key_path, "' not found");
        }
...
bool server::start(bool blocking) {
    ...
    SPDLOG_INFO("Starting MCP server on ", host_, ":", port_);
    ...
    http_server_->Post(msg_endpoint_.c_str(), [this](const httplib::Request& req, httplib::Response& res) {
        ...
        SPDLOG_INFO("{}:{} - \"POST {} HTTP/1.1\" {} - Request: {}", req.remote_addr, req.remote_port, req.path, res.status, req.body);
    });

    // Setup SSE endpoint
    http_server_->Get(sse_endpoint_.c_str(), [this](const httplib::Request& req, httplib::Response& res) {
        this->handle_sse(req, res);
        SPDLOG_INFO("{}:{} - \"GET {} HTTP/1.1\" {}", req.remote_addr, req.remote_port, req.path, res.status);
    });
        ...
        maintenance_thread_ = std::jthread([this](std::stop_token st) {
            ...
            while (!st.stop_requested()) {
                ...
                try {
                    ...
                    SPDLOG_ERROR("Exception in maintenance thread: {}", e.what());
                } catch (...) {
...
void server::stop() {
    ...
    for (auto& session_id : sessions_to_close) {
    ...
    SPDLOG_INFO("MCP server stopped");
}
...
void server::handle_sse(const httplib::Request& req, httplib::Response& res) {
    ...
    auto thread = std::jthread([this, session_id, session_uri, session_dispatcher](std::stop_token st) {
        try {
            ...
            while (running_ && !st.stop_requested() && !session_dispatcher->is_closed()) {
                ...
                if (!session_dispatcher->send_event(heartbeat.str())) {
                    SPDLOG_WARN("Failed to send heartbeat, closing session {}", session_id);
                    break;
                }
                session_dispatcher->update_activity();
            }
        } catch (const std::exception& e) {
            SPDLOG_ERROR("SSE session thread exception: {} ({})", session_id, e.what());
        }
        close_session(session_id);
    });
    ...
    res.set_chunked_content_provider("text/event-stream", [this, session_id, session_dispatcher](size_t /* offset */, httplib::DataSink& sink) {
        try {
            ...
            if (!result) {
                SPDLOG_WARN("Failed to wait for event, closing connection: {}",session_id);
            ...
            SPDLOG_ERROR("SSE content provider exception: {}",e.what());
...
void server::handle_jsonrpc(const httplib::Request& req, httplib::Response& res) {
    ...
    try {
        ...
        SPDLOG_ERROR("Failed to parse JSON request: {}",e.what());
        res.status = 400;
    ...
    std::shared_ptr<event_dispatcher> dispatcher;
    {
        ...
        if (disp_it == session_dispatchers_.end()) {
            ...
            SPDLOG_ERROR("Session not found: {}",session_id);
            res.status = 404;
    ...
    try {
        ...
        SPDLOG_ERROR("Failed to create request object: {}",e.what());
        res.status = 400;
    ...
    thread_pool_.enqueue([this, mcp_req, session_id, dispatcher]() {
        ...
        if (!result) {
            SPDLOG_ERROR("Failed to send response via SSE: session_id=", session_id);
        }
    });
...
json server::process_request(const request& req, const std::string& session_id) {
    ...
    // Process method call
    try {
        SPDLOG_INFO("Processing method call: {}",req.method);
        ...
        if (!is_session_initialized(session_id)) {
            SPDLOG_WARN("Session not initialized: {}",session_id);
        ...
        if (handler) {
            // Call handler
            SPDLOG_INFO("Calling method handler: {}",req.method);
            json result = handler(req.params, session_id);

            // Create success response
            SPDLOG_INFO("Method call successful: {}",req.method);
            return response::create_success(req.id, result).to_json();
        }

        // Method not found
        SPDLOG_WARN("Method not found: {}",req.method);
        return response::create_error(
            req.id,
        ...
        // MCP exception
        SPDLOG_ERROR("MCP exception: {}",e.what(), ", code: {}",static_cast<int>(e.code()));
        return response::create_error(
            req.id,
            e.code(),
            e.what()
        ...
        // Other exceptions
        SPDLOG_ERROR("Exception while processing request: {}",e.what());
        return response::create_error(
            req.id,
        ...
        // Unknown exception
        SPDLOG_ERROR("Unknown exception while processing request");
        return response::create_error(
            req.id,
            error_code::internal_error,
...
json server::handle_initialize(const request& req, const std::string& session_id) {
    ...
    if (!params.contains("protocolVersion") || !params["protocolVersion"].is_string()) {
        SPDLOG_ERROR("Missing or invalid protocolVersion parameter");
        return response::create_error(
            req.id,
    ...
    SPDLOG_INFO("Client requested protocol version: {}",requested_version);
    ...
    if (!version_supported) {
        SPDLOG_ERROR("Unsupported protocol version: {}",requested_version, ", server supports: 2024-11-05, 2025-03-26, 2025-06-18, 2025-11-25");
        return response::create_error(
            req.id,
            error_code::invalid_params,
    ...
    // Log connection
    SPDLOG_INFO("Client connected: {}",client_name, " ", client_version);
    ...
    SPDLOG_INFO("Initialization successful, waiting for notifications/initialized notification");
...
void server::send_jsonrpc(const std::string& session_id, const json& message) {
    ...
    if (session_id.empty()) {
        SPDLOG_WARN("Cannot send message to empty session_id");
        return;
    }
    ...
    if (!dispatcher || dispatcher->is_closed()) {
        SPDLOG_WARN("Cannot send to closed session: {}",session_id);
        return;
    }
    ...
    if (!result) {
        SPDLOG_ERROR("Failed to send message to session: {}",session_id);
    }
}
...
bool server::is_session_initialized(const std::string& session_id) const {
    ...
    try {
        ...
        SPDLOG_ERROR("Exception checking if session is initialized: {}",e.what());
        return false;
    }
}
...
void server::set_session_initialized(const std::string& session_id, bool initialized) {
    // Check if session ID is valid
    if (session_id.empty()) {
        SPDLOG_WARN("Cannot set initialization state for empty session_id");
        return;
    }
    ...
    try {
        ...
        auto it = session_dispatchers_.find(session_id);
        if (it == session_dispatchers_.end()) {
            SPDLOG_WARN("Cannot set initialization state for non-existent session: {}",session_id);
            return;
        }
        session_initialized_[session_id] = initialized;
    } catch (const std::exception& e) {
        SPDLOG_ERROR("Exception setting session initialization state: {}",e.what());
    }
}
...
void server::check_inactive_sessions() {
    ...
    for (const auto& session_id : sessions_to_close) {
        SPDLOG_INFO("Closing inactive session: {}",session_id);
...
void server::close_session(const std::string& session_id) {
    try {
        ...
        {
            std::lock_guard<std::mutex> lock(mutex_);
            if (auto dispatcher_it = session_dispatchers_.find(session_id); dispatcher_it != session_dispatchers_.end()) {
                dispatcher_to_close = dispatcher_it->second;
                session_dispatchers_.erase(dispatcher_it);
            }

            if (auto thread_it = sse_threads_.find(session_id); thread_it != sse_threads_.end()) {
        ...
        if (have_thread && thread_to_join.joinable()) {
            if (thread_to_join.get_id() == std::this_thread::get_id()) {
                thread_to_join.detach();
            } else {
                thread_to_join.request_stop();
                thread_to_join.join();
            }
        }
    } catch (const std::exception& e) {
        SPDLOG_WARN("Exception while cleaning up session resources: {}", session_id, ", ", e.what());
    } catch (...) {
        SPDLOG_WARN("Unknown exception while cleaning up session resources: {}", session_id);
    }
}
